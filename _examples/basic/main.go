...
func main() {
	...
	c.Set("user:123", "David Kier", 5*time.Minute)
	c.Set("user:456", "Michael Ballack", 5*time.Minute)
	c.Set("user:789", "Cristiano Bombaldo", 5*time.Minute)
	...
	c.SetWithCallback("temp:data", "temporary value", 2*time.Second, func(key string, value string) {
